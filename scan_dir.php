...
if(!webAuthenticate("boxcore","123456"))        //验证用户
{
    die();
}
//2005-4-11
//显示当前目录下的文件
$_CONFIG["SiteName"]="文件下载系统 by  boxcore  ";        //网站名称
$_CONFIG["SiteUrl"]="http://www.boxcore.org";            //网站地址
...
while($tmp_Str=$DIRObject->read())
{
    if($tmp_Str!="."&&$tmp_Str!="..")
    {
        ...
        {
            if(strstr($tmp_Str,".phpx") || strstr($tmp_Str,".asp")  )    //不显示 .php .asp的文件
                continue;
            print("<td align=center>");
            print(strftime("%Y-%m-%d %H:%M:%S",filemtime($_DIR_PATH.$tmp_Str)));
            print(" </td>");
